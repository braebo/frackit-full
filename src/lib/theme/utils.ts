...
const log = verbose // Colorful console.log with optional theming, i.e.:  log(`x = ${x})`, 'purple', 20, 'blue')
	? (str, color = 'lightblue', font_size = 15, border = 'gray') =>
			console.log(
				`%c${str}`,
				`size:${font_size}px;color:${color};border:1px solid ${border};padding:5px;`
			)
	: (s, c, f, b) => {}
...
const supports_color_scheme =
	client && window.matchMedia('(prefers-color-scheme)').media !== 'not all'
